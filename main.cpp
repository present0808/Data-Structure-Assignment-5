...
void main() 
{
	...
	do
	{
		...
		{
			_tprintf(TEXT("  %s\n"), ffd.cFileName);
			if (ffd.cFileName[0] == 't' && ffd.cFileName[1] == 'e' && ffd.cFileName[2] == 's' && ffd.cFileName[3] == 't')
			{
				BSTPtr bst = bst_alloc();
				FILE *input;
				int a = 0, nodenum = 0, blacknodenum = 0, bh = 0, insert=0, del=0, miss=0;
				NodePtr a_del;
				fopen_s(&input, ConvertWCtoC(ffd.cFileName), "r");
				for (;;)
				{
					fscanf_s(input, "%d\n", &a);
					if (a>0)
